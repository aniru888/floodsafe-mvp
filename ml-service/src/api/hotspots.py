...
from ..data.fhi_calculator import calculate_fhi_for_location, get_fhi_calculator
...
async def _calculate_hotspots_fhi_bulk(hotspots: List[Dict]) -> Dict[int, Dict]:
    calculator = get_fhi_calculator()
    if not hotspots:
        return {}

    lats = [float(h["lat"]) for h in hotspots]
    lngs = [float(h["lng"]) for h in hotspots]
    ...
    async with httpx.AsyncClient(timeout=calculator._timeout) as client:
        elevation_resp, weather_resp = await asyncio.gather(
            client.get(
                calculator.ELEVATION_URL,
                params={
                    "latitude": ",".join(f"{lat:.4f}" for lat in lats),
                    "longitude": ",".join(f"{lng:.4f}" for lng in lngs),
                },
            ),
            client.get(calculator.FORECAST_URL, params=weather_params),
        )
    ...
    if not isinstance(elevations, list):
        elevations = []
    if len(elevations) != len(hotspots):
        elevations = [220.0] * len(hotspots)
    ...
    for idx, elevation in enumerate(elevations):
        ...
        fhi_score = min(1.0, max(0.0, float(fhi_raw)))

        if precip_3d_raw < calculator.MIN_RAIN_THRESHOLD_MM:
            fhi_score = min(fhi_score, calculator.LOW_FHI_CAP)
...
async def get_all_hotspots(
    ...
    if include_fhi and not test_fhi_override:
        ...
        try:
            fhi_results = await _calculate_hotspots_fhi_bulk(hotspots_data)
        except Exception as e:
            logger.warning(f"Bulk FHI calculation failed, falling back to parallel: {e}")

            semaphore = asyncio.Semaphore(20)
            tasks = [
                _calculate_single_hotspot_fhi(hotspot, idx, semaphore, timeout_seconds=3.0)
        ...
        logger.info(f"FHI calculation completed: {len(fhi_results)}/{len(hotspots_data)} in {elapsed:.2f}s")
...
def initialize_hotspots_router():
    ...
    ml_service_root = Path(__file__).resolve().parent.parent.parent
    apps_root = ml_service_root.parent
    candidate_model_dirs = [
        ml_service_root / "models" / "xgboost_hotspot",
        apps_root / "backend" / "models" / "xgboost_hotspot",
    ]

    model_dir: Optional[Path] = None
    for candidate in candidate_model_dirs:
        if (candidate / "xgboost_model.json").exists() and (candidate / "metadata.json").exists():
            model_dir = candidate
            break
    ...
    if model_dir is not None:
        try:
            ...
            hotspot_model = load_trained_model(model_dir)
            logger.info(f"XGBoost hotspot model loaded from {model_dir}")
        except Exception as e:
    ...
    if not predictions_cache and hotspot_model is not None and hotspot_model.is_trained:
        training_data_path = ml_service_root / "data" / "hotspot_training_data.npz"
        if training_data_path.exists():
            try:
                data = np.load(training_data_path, allow_pickle=True)
                features = data["features"]
                samples = json.loads(str(data["metadata"]))

                hotspot_samples: Dict[int, List[int]] = defaultdict(list)
                for i, sample_meta in enumerate(samples):
                    try:
                        label = int(sample_meta.get("label", 0))
                        sample_id = int(sample_meta["sample_id"])
                    except Exception:
                        continue
                    if label == 1:
                ...
                for hotspot in hotspots_data:
                    ...
                    if sample_indices:
                        ...
                        probs = hotspot_model.predict_proba(hotspot_features)
                        base_susceptibility = float(np.mean(probs))
                    else:
                        base_susceptibility = float(
                            severity_map.get(hotspot.get("severity_history", "moderate"), 0.5)
                        )

                    generated_cache[str(hotspot_id)] = {
                ...
                predictions_cache = generated_cache
                logger.info(
                    f"Generated XGBoost predictions for {len(predictions_cache)} hotspots from training data"
                )
            except Exception as e:
                logger.warning(f"Failed to generate predictions cache from training data: {e}")
