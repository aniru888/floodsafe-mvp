...
async def admin_login(request: AdminLoginRequest, db: Session = Depends(get_db)):
    ...
    # Check password — support both plain-text default and hashed passwords
...
async def update_user_role(
    ...
    from src.infrastructure.models import RoleHistory
    ...
    if role_update.new_role == "verified_reporter":
        user.verified_reporter_since = user.verified_reporter_since or __import__("datetime").datetime.utcnow()
    elif role_update.new_role == "moderator":
        user.moderator_since = user.moderator_since or __import__("datetime").datetime.utcnow()
    ...
    admin_service.log_admin_action(
        db, admin.id, "change_role", "user", str(user_id),
        json.dumps({"old": old_role, "new": role_update.new_role, "reason": role_update.reason})
    )

    db.commit()
    return {"success": True, "old_role": old_role, "new_role": role_update.new_role}
